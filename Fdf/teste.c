
#include <stdlib.h>
#include <mlx.h>

// int	main (void)
// {
// 	void	*mlx;
// 	void	*mlx_win;
// //	void	*img;

// mlx = mlx_init(); // initialize the display. return is void
// mlx_win = mlx_new_window(mlx, 1920, 1080, "hello world!"); //initialize the window. it returns a pointer to the window
// mlx_loop(mlx); //initiate window rendering
// //img = mlx_new_image(mlx, 1920, 1080); //initialize image
// }



typedef struct	s_data {
	void	*img;
	char	*addr;
	int		bits_per_pixel;
	int		line_length;
	int		endian;
}				t_data;

void	my_mlx_pixel_put(t_data *data, int x, int y, int color)
{
	char	*dst;

	dst = data->addr + (y * data->line_length + x * (data->bits_per_pixel / 8));
	*(unsigned int*)dst = color;
}

// int	main(void)
// {
// 	void	*mlx;
// 	void	*mlx_win;
// 	t_data	img;
// 	int	y;
// 	int x;
// 	y = 50;
// 	x = 300;
// 	mlx = mlx_init();
// 	mlx_win = mlx_new_window(mlx, 800, 500, "Hello world!");
// 	img.img = mlx_new_image(mlx, 800, 500);
// 	img.addr = mlx_get_data_addr(img.img, &img.bits_per_pixel, &img.line_length,
// 								&img.endian);
// 	while (y < 200)
// 		my_mlx_pixel_put(&img, 300, y++, 0x0000FF00);
// 	while (x < 450)
// 		my_mlx_pixel_put(&img, x++, y, 0x00FF0000);
// 	while (y > 50)
// 		my_mlx_pixel_put(&img, x, y--, 0x00FF00FF);
// 	while (x > 300)
// 		my_mlx_pixel_put(&img, x--, 50, 0x00FF0000);
// 	mlx_put_image_to_window(mlx, mlx_win, img.img, 0, 0);
// 	mlx_loop(mlx);
// }

int	main(void)
{
	void	*mlx;
	void	*mlx_win;
	t_data	img;
	int	y;
	int x;

	y = 50;
	x = 300;

	mlx = mlx_init();
	mlx_win = mlx_new_window(mlx, 1080, 1080, "Hello world!");
	img.img = mlx_new_image(mlx, 1080, 1080);
	img.addr = mlx_get_data_addr(img.img, &img.bits_per_pixel, &img.line_length,
								&img.endian);
	while (y < 250)
	{
		x = 300;
		while (x < 500)
		{
			my_mlx_pixel_put(&img, x++, y, 0x0000FF00);
		}
		y++;	
	}
	mlx_put_image_to_window(mlx, mlx_win, img.img, 0, 0);
	mlx_loop(mlx);
}